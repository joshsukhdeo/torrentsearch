(function () {
    ...
    program
        .version(pkg.version)
        .option('--config', 'Edit anujyadav config EX: anujyadav --config="nano"')
        ...
        //.option('--history', 'View anujyadav history EX: anujyadav --history')
        .option('--clear', 'Clear anujyadav history EX anujyadav --clear')
        //.option('--location', 'Change where anujyadav\' config & history is located EX anujyadav --location"/home/gshock/.config"')
        .parse(process.argv);
    ...
    if (program.config) {
        if (program.args[0]) {
            ...
            var edit_conf;
            var conf = new Configstore("anujyadav_config", {});
            edit_conf = conf.path;

            if (conf.size < 1) {
              console.log(chalk.red('A config file has not been created yet.'));
              console.log(chalk.red('Please run anujyadav at least once to create one.'));
            } else {
            ...
            console.log("  Example:");
            console.log('');
            console.log('    $ anujyadav --config="nano"');
        }
        ...
        //do an update check
        var url = "https://raw.githubusercontent.com/whoisashish/anujyadav/master/package.json";
        request(url, function (err, response, body) {
            if (!err && response.statusCode === 200) {
                data = JSON.parse(body);
                if (data.version > pkg.version) {
                    console.log(chalk.red('A new version of anujyadav is available run:'));
                    console.log("");
                    console.log('$ npm install -g anujyadav');
                    console.log("");
                    console.log(chalk.red('Or go here to get it: https://github.com/whoisashish/anujyadav'));
                }
            }
    ...
    function AppInitialize(prog) {
        ...
        var conf = new Configstore("anujyadav_config", {});
        var hist = new Configstore("anujyadav_history", { recentlyWatched: { items: [] }});
        if (conf.size < 1) {
