(function() {
    ...
    var get_conf_object = new Configstore("ashishyadav_config", {});
    var get_hist_object = new Configstore("ashishyadav_history", {});
    ...
    exports.AppInitialize = start = function(optns) {
      ...
      hide_inactive_torrents = config_object.options.hide_inactive_torrents === "true";
      sort_by = config_object.options.default_sort_by ?? "";
      sort_asc = config_object.options.default_sort_asc === "true";
      sort_direction = (sort_asc) ? 1 : -1;
    ...
    function onResolve(initial_data) {
        var data = (hide_inactive_torrents) ? initial_data.filter(torrent => torrent.seeds > 0) : initial_data;
        var inactive_torrent_count = initial_data.length - data.length;
        if(sort_by != ""){
          data.sort((a,b) => (a[sort_by] - b[sort_by]) * sort_direction)
        }
        console.log("sort_by: " + chalk.cyanBright(sort_by) + 
          ", hide_inactive_torrents: " + chalk.cyanBright(hide_inactive_torrents) +
          "\n" + "sort_asc: " + chalk.cyanBright(sort_asc) + 
          ", sort_direction: " + chalk.cyanBright(sort_direction) +
          "\n" + chalk.white('-------------------------------------------------')
        );
        
        var current_torrent_index = 0;
        for (var idx in data) {
            var torrent = data[idx];
            var date_added, title;
            var number = (sort_by != "") ? ++current_torrent_index : torrent.torrent_num;
        ...
        if(inactive_torrent_count > 0){
          console.log("~~~ " + chalk.yellow('Omitting ') + chalk.green(inactive_torrent_count) + chalk.yellow(' inactive torrents with no seeders') + " ~~~")
        }
        selectTorrent(data);
    }
    ...
    function streamTorrent_wosub(torrent){
      ...
      if(peerflix_player === "--airplay"){
        ...
        if (isWindows) {
          argsList =  [torrent.split('&')[0], peerflix_player, peerflix_player_arg, peerflix_port];
        } else{
    ...
    function streamTorrent_sub(torrent){
      ...
      if(peerflix_player === "--airplay"){
        ...
        if (isWindows) {
          argsList = [torrent.split('&')[0], "--subtitles=\"" + peerflix_subtitle + "\"", peerflix_player, peerflix_player_arg, peerflix_port];
        } else {
    ...
    function osSpecificSpawn(command, argsList){
      ...
      if (isWindows) {
        ...
        const peerflix = spawn('cmd', argsList, {stdio:'inherit'});

        peerflix.on('close', (code) => {
          if (code == 0) {
            trakt.markaswatched(torrent_title);
        ...
        const peerflix = spawn(command, argsList, {stdio:'inherit'});
